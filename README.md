...
* Check the `ConnectionStrings` in `appsettings.json` files under the `BookAuthorDetials.Web` and `BookAuthorDetials.DbMigrator` projects and change it if you need.
...
* Run `BookAuthorDetials.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 136aad65-4fea-4e81-9421-656ba651979b
```
...
* `BookAuthorDetials.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `BookAuthorDetials.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
